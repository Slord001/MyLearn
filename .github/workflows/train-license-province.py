...
x = tf.placeholder(tf.float32, shape=[None, SIZE]) #给x一个占位符
y_ = tf.placeholder(tf.float32, shape=[None, NUM_CLASSES])#给y一个占位符
    ...
    for i in range(0,NUM_CLASSES):
        ...
        for rt, dirs, files in os.walk(dir):
            for filename in files:
                ...
                #img_new = Image.open(dir+"new.bmp")#slord
                #img_new = img_new.convert('1') #slord
                width = img.size[0]
                height = img.size[1]
                #print(dir+"new.bmp") #slord
                for h in range(0, height):
                    for w in range(0, width):
                        ...
                        if img.getpixel((w, h)) > 230:
                            ...
                            #img_new.putpixel((w,h),0)  #slord
                            #img_new.putpixel((w,h),input_images[index,w+h*width])
                        else:
                            input_images[index][w+h*width] = 1
                            #img_new.putpixel((w,h),1)  #slord
                        
                        #img_new.putpixel((w,h),input_images[index,w+h*width]) #slord
                          
                        
                input_labels[index][i] = 1
                index += 1
                #img_new.save(dir+"new.bmp")
                #sys.exit(0)  #slord
...
if __name__ =='__main__' and sys.argv[1]=='predict':
    ...
    with tf.Session() as sess:
        ...
        print ("省份可能是: %s" % PROVINCES[nProvinceIndex])
